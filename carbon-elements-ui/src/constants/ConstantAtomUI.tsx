import * as React from 'react';
import { PortWidget } from '@projectstorm/react-diagrams-core';
import { ConstantAtomWidget } from './ConstantAtomUIWidget';
import { StormAtomUI, StormAtomUIRenderEvent, StormPortWidget } from '@journeyapps-labs/carbon-ui-storm';
import {
  BaseConstantAtom,
  NumericConstantAtomFactory,
  StringConstantAtomFactory,
  ValueType
} from '@journeyapps-labs/carbon-elements';
import { CarbonTheme } from '@journeyapps-labs/carbon-ui';

export class ConstantAtomUI extends StormAtomUI {
  render(event: StormAtomUIRenderEvent<BaseConstantAtom>): React.JSX.Element {
    return (
      <ConstantAtomWidget
        uiBank={this.bank}
        node={event.event.model}
        getPortTheme={(theme: CarbonTheme) => {
          if (this.type === NumericConstantAtomFactory.TYPE) {
            return theme.types[ValueType.NUMBER];
          }
          if (this.type === StringConstantAtomFactory.TYPE) {
            return theme.types[ValueType.TEXT];
          }
          return theme.types[ValueType.TEXT];
        }}
        validate={() => true}
        engine={event.engine}
        readonly={event.readonly}
        renderPort={(port, element) => {
          return (
            <StormPortWidget port={port} engine={event.engine} model={event.event.model}>
              {element}
            </StormPortWidget>
          );
        }}
        atom={event.event.model.atom}
      />
    );
  }
}
